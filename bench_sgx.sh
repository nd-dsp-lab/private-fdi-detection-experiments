...
# SGX Smart Grid Benchmark Launcher
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
...
# Globals and defaults (needed for set -u)
PIDS=()
SERVER_PID=""
CLEANUP_DONE=false
...
# SGX wrapper + app (manifest basename, not a path)
SERVER_WRAPPER="gramine-sgx"
SERVER_BIN="smart_grid_server"   # This is the correct manifest base name
...
print_banner() {
  echo -e "\n${CYAN}${BOLD}=============================================="
  echo -e "      SGX SMART GRID BENCHMARK LAUNCHER"
  echo -e "==============================================${NC}\n"
}
...
check_deps() {
  ...
  # SGX wrapper
  if ! command -v "$SERVER_WRAPPER" >/dev/null 2>&1; then
    echo -e "${RED}[ERROR]${NC} $SERVER_WRAPPER not found"
    return 1
  fi

  # For Gramine, we need the signed manifest, not an -x binary
  if [ ! -f "${SERVER_BIN}.manifest.sgx" ]; then
    if $NO_BUILD; then
      echo -e "${RED}[ERROR]${NC} ${SERVER_BIN}.manifest.sgx not found and --no-build set"
      return 1
    fi
    echo -e "${YELLOW}[BUILD]${NC} Building SGX server and manifest..."
    if [ -x ./build_sgx.sh ]; then
      ./build_sgx.sh
    else
      echo -e "${RED}[ERROR]${NC} build_sgx.sh not found; build the SGX target and manifest manually"
      return 1
    fi
    if [ ! -f "${SERVER_BIN}.manifest.sgx" ]; then
      echo -e "${RED}[ERROR]${NC} ${SERVER_BIN}.manifest.sgx still missing after build"
      return 1
    fi
  fi
...
start_server() {
  ...
  local args=( "$SERVER_WRAPPER" "$SERVER_BIN"
               "--port" "$port"
               "--devices" "$devices"
  ...
  echo -e "${CYAN}[DEBUG]${NC} Running: ${args[*]}"
  ...
  sleep 3  # Give more time for SGX initialization

  if kill -0 "$SERVER_PID" 2>/dev/null; then
    echo -e "${GREEN}[OK]${NC} SGX Server started (PID: $SERVER_PID)"
    return 0
  else
    echo -e "${RED}[ERROR]${NC} Failed to start SGX server"
    return 1
  fi
}
